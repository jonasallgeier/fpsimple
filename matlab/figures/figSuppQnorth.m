...
Q1              = (h1-h2)./L.*Tx.*(wMax-wMin);

% create initial models
mdl = fpAna('h1',h1,'h2',h2,'L',L(1),'wMin',wMin(1),...
          'wMax',wMax(1),'Tx',Tx,'Ty',Ty,...
          'qNorth',Rstar(1).*Q1(1)./L(1),'shape',shape,'autoSolve',false);
  ...
  for i2 = 1:size(wMin,2)
    for i3 = 1:size(wMax,3)
      ...
      for i4 = 1:size(Rstar,4)
        % modify Ky
        [~]                 = set(mdl,'qNorth',Rstar(i1,i2,i3,i4).*Q1(i1,i2,i3,i4)./L(i1,i2,i3,i4));
...
% y               = sqrt(Qstar(:,:,:,1)) - sqrt(Qstar);
% y               = y.*m.*(h1-h2).*sqrt(Kx.*Ky)./sqrt(wMin.*wMax).*(wMax-wMin);
% y               = y./abs(R);
...
% SOMETHING THAT BECOMES LINEAR AT X=0 (...^x)
...
% modelB        =  @(p,x) p(1)+x.^4.*p(2)+p(3)*x.*x;
