function [success] = modelSummary(mdl)
  % print a summary of a model, including (intermediate) results of
  % simplified proxy-model calculation
  
  % the fitted parameters are hard-coded; values obtained with figInfluxNorth
  switch mdl.shape
    case "bump"
      a1 = 5.852;
      a2 = 0.355;
  ...
  % determine various (intermediate) results
  ...
  % assign dummy output to indicate that nothing went wrong
  success = true;
end